...
### Machine learning algorithms [and most common operations with time complexities, real world examples]

**Machine Learning algorithms** learn patterns from data to make predictions or decisions. Here are key algorithms with their data structure implementations.

#### Supervised Learning Algorithms

**1. K-Nearest Neighbors (KNN) - O(n) prediction**
```java
import java.util.*;
...
public class MachineLearningAlgorithms {
    ...
    // 2. Decision Tree - O(n log n) training, O(log n) prediction
...
public class NeuralNetwork {
    ...
    static class Perceptron {
        ...
        public void train(List<double[]> inputs, List<Integer> targets, int epochs) {
        ...
        public MLP(int inputSize, int hiddenSize, int outputSize, double learningRate) {
            ...
            for (int i = 0; i < inputSize; i++) {
                for (int j = 0; j < hiddenSize; j++) {
                    hiddenWeights[i][j] = random.nextGaussian() * 0.1;
                }
            }
        ...
        public double[] predict(double[] input) {
            ...
            for (int j = 0; j < output.length; j++) {
                ...
                for (int i = 0; i < hidden.length; i++) {
                    output[j] += hidden[i] * outputWeights[i][j];
                }
...
| Perceptron | O(n×epochs) | O(d) | O(d) | Linear classification |
| MLP | O(n×epochs×layers) | O(layers×nodes) | O(layers×nodes) | Non-linear problems |
...
**Classification:**
- **Email Spam Detection**: Text classification using Naive Bayes
- **Image Recognition**: CNNs for object detection
- **Medical Diagnosis**: Decision trees for symptom analysis

**Clustering:**
- **Customer Segmentation**: K-means for marketing analysis
- **Gene Expression**: Hierarchical clustering in bioinformatics  
- **Social Network Analysis**: Community detection algorithms

**Regression:**
- **Stock Price Prediction**: Time series forecasting
- **Recommendation Systems**: Collaborative filtering
- **Autonomous Vehicles**: Neural networks for control systems

**Optimization:**
- **Route Planning**: Genetic algorithms for TSP
- **Hyperparameter Tuning**: Grid search, random search
- **Feature Selection**: Evolutionary algorithms
